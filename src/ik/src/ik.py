...
def ik_service_client():
    ...
    while True:
        try:
            posn_x, posn_y, posn_z = map(float, input("Enter EF position (, separated): ").replace(' ', "").split(','))
            break
    ...
    pose_stamped.pose.position.x = posn_x
    pose_stamped.pose.position.y = posn_y
    pose_stamped.pose.position.z = posn_z
    
    # Set end effector quaternion: YOUR CODE HERE
    q_x, q_y, q_z, q_w = 0.0, 1.0, 0.0, 0.0 # Hint #2
