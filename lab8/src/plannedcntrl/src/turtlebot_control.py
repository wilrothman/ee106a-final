...
def controller(waypoint):
  ...
  while not rospy.is_shutdown():
    try:
      ...
      # integral term
      dt = 0 # TODO: quick operation to determine dt
      integ += np.zeros(2) # TODO: integral is summing up error over time, so what would we expect to add on to our integral term tracker here?
      integral = np.dot(Ki, integ).squeeze()

      # dervative term
      error_deriv = (error-previous_error)/(float(curr_time - prev_time)) # TODO: quick operation to determine dt
      ...
      if np.abs(np.sum(error)) < 0.1: #TODO: what is our stopping condition/how do we know to go to the next waypoint?
